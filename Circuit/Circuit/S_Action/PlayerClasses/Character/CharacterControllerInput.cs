...
namespace Circuit.S_Action.PlayerClasses.Character
{
    /// <summary>
    ...
    public class CharacterControllerInput
    {
        ...
        public CharacterController CharacterController
        {
            ...
            set { mCharacterController = value; }
        }
        ...
        public Collidable CollisionInformation { get { return mCharacterController.Body.CollisionInformation; } }
        ...
        public CharacterControllerInput(Space owningSpace)
        {
            mCharacterController = new CharacterController(new Vector3(), Data.P_HEIGHT, 1.7f * .7f, Data.P_RADIUS, Data.P_MASS);

            Space = owningSpace;
            
            Space.Add(mCharacterController);
        ...
        public void Activate()
        {
            ...
            {
                IsActive = true;
                //CameraPos.UseMovementControls = false;
                Space.Add(mCharacterController);
                //CharacterController.Body.Position = (CameraPos.Position - new Vector3(0, StandingCameraOffset, 0));
            }
        }
        ...
        public void Deactivate()
        {
            ...
            {
                IsActive = false;
                //CameraPos.UseMovementControls = true;
                Space.Remove(mCharacterController);
            }
        }
        ...
        public void Update(GameTime _GT)
        {
            if (IsActive)
            {
                ...
                if (true)
                {
                    ...
                    Position = Position + mCharacterController.Body.LinearVelocity * (float)_GT.ElapsedGameTime.TotalSeconds;
                   

                    //Now compute where it should be according the physical body of the character.
                    Vector3 up = mCharacterController.Body.OrientationMatrix.Up;
                    Vector3 bodyPosition = mCharacterController.Body.BufferedStates.InterpolatedStates.Position;
                    Vector3 goalPosition = bodyPosition + up * (mCharacterController.StanceManager.CurrentStance == Stance.Standing ? StandingCameraOffset : CrouchingCameraOffset);
                    ...
                    if (verticalError > mCharacterController.StepManager.MaximumStepHeight)
                    {
                        Position-= up * (mCharacterController.StepManager.MaximumStepHeight - verticalError);
                        verticalError = mCharacterController.StepManager.MaximumStepHeight;
                    }
                    else if (verticalError < -mCharacterController.StepManager.MaximumStepHeight)
                    {
                        Position-= up * (-mCharacterController.StepManager.MaximumStepHeight - verticalError);
                        verticalError = -mCharacterController.StepManager.MaximumStepHeight;
                    }
                    ...
                    float margin = mCharacterController.Body.CollisionInformation.Shape.CollisionMargin;
                    ...
                    float errorCorrectionFactor = (float)(1 - Math.Pow(.000000001, (float)_GT.ElapsedGameTime.TotalSeconds));
                ...
                {
                    Position = mCharacterController.Body.Position + (mCharacterController.StanceManager.CurrentStance == Stance.Standing ? StandingCameraOffset : CrouchingCameraOffset) * mCharacterController.Body.OrientationMatrix.Up;
                }
                ...
                {
                    movementDir = CameraPos.WorldMatrix.Forward;
                ...
                {
                    movementDir = CameraPos.WorldMatrix.Left;
                    totalMovement += Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
                }
                if (keyboardInput.IsKeyDown(Keys.F))
                {
                    movementDir = CameraPos.WorldMatrix.Right;
                    totalMovement += Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
                }
        ...
        public void Move(Vector2 _total)
         {
            mCharacterController.HorizontalMotionConstraint.MovementDirection = Vector2.Normalize(_total);
        }
    }
}